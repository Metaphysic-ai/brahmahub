...
import { FolderOpen, User, Package, Database, Film } from 'lucide-react';
...
export function GlobalSearch({ open, onOpenChange }: GlobalSearchProps) {
  ...
  const hasResults = results && (
    results.projects.length || results.subjects.length || results.packages.length || results.assets.length
  );

  return (
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) setQuery(''); onOpenChange(isOpen); }}>
