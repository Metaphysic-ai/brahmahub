  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 rounded-lg border border-border/40 bg-background/50 p-2 group">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button onClick={handleCopy} className="shrink-0 text-muted-foreground/50 hover:text-foreground transition-colors duration-200">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button onClick={handleCopyDgx} className="shrink-0 text-muted-foreground/50 hover:text-foreground transition-colors duration-200">
              {copied === 'dgx' ? <Check size={14} /> : <Server size={14} />}
            </button>
          </TooltipTrigger>
          <TooltipContent side="top" className="text-xs">Copy as DGX path</TooltipContent>
        </Tooltip>
      </div>
    </div>
  );
