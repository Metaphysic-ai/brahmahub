...
import { usePaginatedAssets, useToggleAssetPickedUp, useUpdateAssetTags } from "@/hooks/useAssets";
import { useDebounce } from "@/hooks/useDebounce";
import { useDeletePackage, usePackage, usePackageSummary, useUpdatePackage } from "@/hooks/usePackages";
  ...
  return (
    <div className="flex flex-col gap-1.5 group">
      ...
      >
        ...
        >
          <div
            className={`rounded-md p-1 transition-all duration-200 ${isSelected ? "bg-primary/90 shadow-md" : "bg-background/70 backdrop-blur-sm"}`}
          >
            <Checkbox
              checked={isSelected}
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-[2] border-l border-border overflow-y-auto p-4 space-y-4">
              ...
              {(() => {
                ...
                return (
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1 text-xs text-muted-foreground">
                    {items.map((item, i) => (
                      <span key={i}>
                        {i > 0 && <span className="text-muted-foreground/30 mr-2">·</span>}
                        {item}
                      </span>
                    ))}
                  </div>
            ...
            <section>
              ...
              <div
                className={`flex items-center gap-1.5 text-xs mb-2 ${asset.is_on_disk ? "text-foreground/70" : "text-destructive"}`}
              >
                {asset.is_on_disk ? (
                  <>
                  ...
                  <>
                    ...
                    <span>Offlined — not on disk</span>
                  </>
                )}
              </div>
            ...
            {(() => {
              const sourcePath = asset.metadata.face?.source_filepath ?? pkg?.metadata.source_video_path;
...
export default function PackageDetail() {
  ...
  const groupedAssets = useMemo(() => {
    ...
    return ASSET_TYPE_ORDER.filter((t) => groups.has(t))
    ...
    <div className="p-5 space-y-5">
      ...
      {(project || subject) && (
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
          {project && (
            <Link to={`/projects/${project.id}`} className="hover:text-foreground transition-colors duration-200">
              {project.name}
            </Link>
          )}
          ...
          {linkedSubjects.length > 0
            ? linkedSubjects.map((s, i) => (
                <span key={s.id} className="flex items-center gap-1.5">
                  ...
                  <Link
                    to={`/projects/${project?.id}/subjects/${s.id}`}
            ...
            : subject && (
                <Link
                  to={`/projects/${subject.project_id}/subjects/${subject.id}`}
                  className="hover:text-foreground transition-colors duration-200"
                >
                  {subject.name}
                </Link>
      ...
      {summary && (
        <PackageSummaryCard
          ...
          onShowGrid={() => setAssetTypeFilter("grid")}
          selectedPoseBins={selectedPoseBins}
          onPoseBinSelectionChange={setSelectedPoseBins}
        />
      )}
      ...
      <Collapsible open={metadataOpen} onOpenChange={setMetadataOpen}>
        ...
        <CollapsibleContent>
          <pre className="mt-2 rounded-lg bg-background/50 p-3 text-xs font-mono-path overflow-x-auto border border-border/30 text-muted-foreground">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="text-xs text-muted-foreground shrink-0">
          ...
          {assetData?.pages[0] && filteredAssets.length < assetData.pages[0].total
            ? ` of ${assetData.pages[0].total.toLocaleString()}`
            : ""}{" "}
          assets
        </span>
        {showSubjectFilter && (
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                className="h-8 text-xs gap-1 min-w-[100px] justify-between rounded-lg"
              >
                {assetTypeFilter === "all"
                  ? "All types"
                  : assetTypeFilter === "aligned"
                    ? "Aligned"
                    : assetTypeFilter === "grid"
                      ? "Grid Video"
                      : "Plate/Source"}
                <ChevronDown size={12} />
              </Button>
            ...
            return (
              <div key={group.type} className="rounded-lg border border-border/20 bg-card/20">
                ...
                >
                  ...
                  <Badge variant="secondary" className="text-2xs px-1.5 py-0 h-4 ml-1">
                    {group.assets.length}
                  </Badge>
                </button>
                {!isCollapsed && (
