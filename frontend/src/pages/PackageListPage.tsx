...
import { Search, Plus, Loader2, type LucideIcon } from 'lucide-react';
...
import { usePaginatedPackages, useBulkDeletePackages } from '@/hooks/usePackages';
...
export default function PackageListPage({ packageType, title, entityLabel, emptyIcon }: PackageListPageProps) {
  ...
  if (isLoading) return <div className="p-5"><Skeleton className="h-8 w-48" /></div>;
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="relative w-64">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder={`Search ${entityLabel}s...`} className="pl-8 h-8 text-xs" />
          </div>
      ...
      {!packages.length ? (
                ...
                <TableRow key={pkg.id} className={selectedIds.has(pkg.id) ? 'bg-primary/10 shadow-[inset_3px_0_0_hsl(var(--primary))]' : ''}>
                  ...
                  <TableCell>
                    {pkg.linked_subjects?.length ? (
                        ...
                        {pkg.linked_subjects.map((s, i) => {
                          ...
                          return (
                            <span key={s.id}>
                              ...
                              <Link to={`/projects/${si?.projectId ?? sub?.projectId}/subjects/${s.id}`} className="text-muted-foreground hover:text-foreground transition-colors duration-200">
                                {s.name}
                              </Link>
                            </span>
                          );
                      ...
                      <Link to={`/projects/${sub.projectId}/subjects/${pkg.subject_id}`} className="text-xs text-muted-foreground hover:text-foreground transition-colors duration-200">
                        {sub.name}
                      </Link>
                    ) : '—'}
                  </TableCell>
                  <TableCell>
                    {sub ? (
                      <Link to={`/projects/${sub.projectId}`} className="text-xs text-muted-foreground hover:text-foreground transition-colors duration-200">
        ...
        {hasNextPage && (
          <div className="flex justify-center pt-2">
            <Button variant="outline" size="sm" className="text-xs gap-1.5" onClick={() => fetchNextPage()} disabled={isFetchingNextPage}>
