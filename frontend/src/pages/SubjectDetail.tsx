...
import { ChevronDown, ChevronRight, Film, ImageIcon, Music, Search, Grid3X3, LayoutGrid, Maximize, Plus, ChevronLeft, ChevronRight as ChevronRightIcon, X, Pencil, Trash2, Copy, Check, Download, Package as PackageIcon, HardDrive, CloudOff } from 'lucide-react';
...
import { useSubject, useDeleteSubject, useUpdateSubject } from '@/hooks/useSubjects';
import { usePackages, useBulkDeletePackages } from '@/hooks/usePackages';
import { usePaginatedAssets, useUpdateAssetTags, useToggleAssetPickedUp, useBulkUpdateAssets } from '@/hooks/useAssets';
...
import { formatBytes, formatDuration, formatResolution, relativeTime, formatFullDate, pluralize } from '@/lib/formatters';
import { getPackageColor } from '@/lib/tagColors';
import { generateManifestJSON, generateCSV, downloadFile, copyPathsToClipboard } from '@/lib/export';
  ...
  return (
    <div className="flex flex-col gap-1.5 group">
      ...
      >
        ...
        >
          <div className={`rounded-md p-1 transition-all duration-200 ${
            isSelected ? 'bg-primary/90 shadow-md' : 'bg-background/70 backdrop-blur-sm'
          }`}>
            <Checkbox checked={isSelected} className="h-3.5 w-3.5 data-[state=checked]:bg-transparent data-[state=checked]:border-primary-foreground" />
          </div>
        </div>
      ...
      <div className="fixed top-0 right-0 z-50 h-full w-[65%] max-w-[1200px] min-w-[600px] bg-card border-l animate-slide-in-right flex flex-col">
        <div className="flex items-center px-3 h-10 border-b shrink-0 gap-2">
          <div className="flex items-center gap-1 shrink-0">
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => currentIndex > 0 && onNavigate(currentIndex - 1)} disabled={currentIndex === 0}>
              <ChevronLeft size={14} />
            </Button>
            <span className="text-xs text-muted-foreground w-12 text-center">{currentIndex + 1} / {assets.length}</span>
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => currentIndex < assets.length - 1 && onNavigate(currentIndex + 1)} disabled={currentIndex === assets.length - 1}>
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="flex-[3] flex items-center justify-center bg-black min-w-0">
            {asset.file_type === 'video' ? (
              <video controls playsInline className="w-full h-full object-contain" src={asset.proxy_url ?? undefined} key={asset.id}>
              ...
              >
                ...
                {imageZoomed && (
                  <Button variant="secondary" size="sm" className="absolute top-2 right-2 text-xs h-6" onClick={(e) => { e.stopPropagation(); setImageZoomed(false); }}>
                    Fit
                  </Button>
                )}
              </div>
          ...
          <div className="flex-[2] border-l border-border overflow-y-auto p-4 space-y-4">
            ...
            <section>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <p className="text-sm font-semibold font-mono-path break-all cursor-default">{asset.disk_path.split('/').pop()}</p>
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-lg break-all font-mono-path text-xs">{displayPath(asset.disk_path)}</TooltipContent>
              </Tooltip>
              <p className="text-2xs text-muted-foreground font-mono-path break-all mt-0.5">{displayPath(asset.disk_path)}</p>
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-xs">
                <span className="text-muted-foreground">Type</span><span>{asset.file_type}</span>
                {asset.codec && <><span className="text-muted-foreground">Codec</span><span>{asset.codec}</span></>}
                <span className="text-muted-foreground">Resolution</span><span>{formatResolution(asset.width, asset.height)}</span>
                {asset.duration_seconds && <><span className="text-muted-foreground">Duration</span><span>{formatDuration(asset.duration_seconds)}</span></>}
                <span className="text-muted-foreground">Size</span><span>{asset.file_size_bytes ? formatBytes(asset.file_size_bytes) : '—'}</span>
              </div>
            </section>

            <section>
              <h3 className="text-xs text-muted-foreground uppercase tracking-wider mb-2">Disk Status</h3>
              <div className={`flex items-center gap-2 rounded-lg border p-2 text-xs ${asset.is_on_disk ? 'border-border/40 bg-background/50' : 'border-destructive/30 bg-destructive/5'}`}>
              {asset.is_on_disk ? (
                  <><HardDrive size={14} className="text-primary shrink-0" /><span className="text-foreground/80">On disk</span></>
                ) : (
                  <><CloudOff size={14} className="text-destructive shrink-0" /><span className="text-destructive">Offlined — not on disk</span></>
                )}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="text-xs space-y-1">
                {asset.camera && <div><span className="text-muted-foreground">Camera: </span>{asset.camera}</div>}
                {pkg && <div><span className="text-muted-foreground">Package: </span><Link to={`/packages/${pkg.id}`} className="text-primary hover:underline">{pkg.name}</Link></div>}
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                {asset.file_type === 'image' && asset.metadata.face?.source_filepath && (
                  <CopyCommandButton label="Copy source video RV" command={`rv "${asset.metadata.face.source_filepath}"`} />
                )}
              </div>
            </section>
...
export default function SubjectDetail() {
  ...
  const packagesLoading = atmanPkgLoading || vfxPkgLoading;
    ...
    <div className="p-4 space-y-4 relative">
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-1.5">
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditName(subject.name); setEditDialogOpen(true); }}><Pencil size={14} /></Button>
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>This will permanently delete "{subject.name}" and all associated packages and assets.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
            ...
            {atmanPkgLoading ? (
              ...
              <>
                <BulkActionBar
                  ...
                  entityLabel="package"
                  onDelete={() => handleBulkDeletePackages([...atmanSelection.selectedIds], atmanSelection.clearSelection)}
                      ...
                      >
                        <TableCell onClick={e => e.stopPropagation()}>
                          <Checkbox
                            ...
                            onClick={(e) => atmanSelection.handleCheckboxChange(pkg, index, e as unknown as React.MouseEvent)}
                            className="h-3.5 w-3.5"
                          />
                        </TableCell>
                        <TableCell>
                          <Link to={`/packages/${pkg.id}`} className="font-mono-path text-xs font-medium text-foreground/90 hover:text-primary transition-colors duration-200" onClick={e => e.stopPropagation()}>
                            {pkg.name}
                          </Link>
                        </TableCell>
                        ...
                        <TableCell>
                          <Tooltip><TooltipTrigger asChild><span className="text-xs text-muted-foreground">{relativeTime(pkg.ingested_at)}</span></TooltipTrigger><TooltipContent>{formatFullDate(pkg.ingested_at)}</TooltipContent></Tooltip>
                        </TableCell>
                        <TableCell>
                          <div className="flex gap-1 flex-wrap">
                            {pkg.tags.slice(0, 3).map(tag => <TagChip key={tag} tag={tag} />)}
                      ...
                      >
                        <TableCell onClick={e => e.stopPropagation()}>
                          <Checkbox
                            ...
                            onClick={(e) => vfxSelection.handleCheckboxChange(pkg, index, e as unknown as React.MouseEvent)}
                            className="h-3.5 w-3.5"
                          />
                        </TableCell>
                        <TableCell>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="text-xs text-muted-foreground shrink-0">
          {filteredAssets.length.toLocaleString()}{assetStats && filteredAssets.length < assetStats.total ? ` of ${assetStats.total.toLocaleString()}` : ''} {filteredAssets.length === 1 ? 'asset' : 'assets'}
        </span>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="h-8 text-xs gap-1 min-w-[140px] justify-between font-mono-path">
              {selectedPackageId ? (packages?.find(p => p.id === selectedPackageId)?.name ?? 'Package') : 'All packages'}
              <ChevronDown size={12} />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="max-h-60 overflow-y-auto">
            ...
            {packages?.map(pkg => (
              <DropdownMenuItem key={pkg.id} onClick={() => setSelectedPackageId(pkg.id)} className="font-mono-path text-xs">
                {pkg.name}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        ...
        <div className="relative max-w-[140px]">
          <Input value={tagFilter} onChange={e => setTagFilter(e.target.value)} placeholder="Filter by tag..." className="h-8 text-xs" />
        </div>
        <div className="ml-auto flex items-center gap-1">
          ...
          <Button variant={gridSize === 'sm' ? 'secondary' : 'ghost'} size="icon" className="h-7 w-7" onClick={() => setGridSize('sm')}>
            <Grid3X3 size={14} />
          </Button>
          <Button variant={gridSize === 'md' ? 'secondary' : 'ghost'} size="icon" className="h-7 w-7" onClick={() => setGridSize('md')}>
            <LayoutGrid size={14} />
          </Button>
          <Button variant={gridSize === 'lg' ? 'secondary' : 'ghost'} size="icon" className="h-7 w-7" onClick={() => setGridSize('lg')}>
            <Maximize size={14} />
          </Button>
        </div>
      </div>
      ...
      {selectedAssetIds.size > 0 && (
        <div className="bg-card border rounded-lg flex items-center gap-3 h-10 px-4">
          <span className="text-xs font-medium">{selectedAssetIds.size} {selectedAssetIds.size === 1 ? 'asset' : 'assets'} selected</span>
          <div className="h-4 w-px bg-border" />
          <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={handleBulkCopyPaths}>
            <Copy size={12} /> Copy paths
          </Button>
          {showBulkTagInput ? (
            <form onSubmit={e => { e.preventDefault(); handleBulkAddTag(); }} className="flex gap-1 items-center">
              <Input value={bulkTagInput} onChange={e => setBulkTagInput(e.target.value)} placeholder="Tag name" className="h-6 w-24 text-xs" autoFocus />
              <Button type="submit" variant="ghost" size="sm" className="h-6 text-xs px-2">Add</Button>
            </form>
          ) : (
          ...
          <Button variant="ghost" size="sm" className="h-7 text-xs ml-auto" onClick={() => setSelectedAssetIds(new Set())}>
            Clear selection
          </Button>
        </div>
      )}
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <Input value={editName} onChange={e => setEditName(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleRenameSubject()} autoFocus />
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleRenameSubject} disabled={!editName.trim() || updateSubjectMut.isPending}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
