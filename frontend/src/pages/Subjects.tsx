    ...
    <div className="p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="relative w-64">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search subjects..." className="pl-8 h-8 text-xs" />
        </div>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {filtered.map((s, index) => (
              <TableRow key={s.id} className={selectedIds.has(s.id) ? 'bg-primary/10 shadow-[inset_3px_0_0_hsl(var(--primary))]' : ''}>
                <TableCell>
                  <Checkbox
                ...
                <TableCell>
                  <Link to={`/projects/${s.project_id}/subjects/${s.id}`} className="font-medium text-sm hover:text-primary transition-colors duration-200">
                    {s.name}
                  </Link>
                </TableCell>
                <TableCell>
                  <Link to={`/projects/${s.project_id}`} className="text-muted-foreground hover:text-foreground transition-colors duration-200 text-xs">
                ...
                <TableCell>
                  <div className="flex gap-1 flex-wrap">
                    ...
                    {s.tags.length > 3 && <span className="text-2xs text-muted-foreground">+{s.tags.length - 3}</span>}
                  </div>
                </TableCell>
