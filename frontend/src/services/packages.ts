...
export const updatePackage = (id: string, data: Record<string, unknown>) => api.put<Package>(`/packages/${id}`, data);

export const deletePackage = (id: string) => api.delete(`/packages/${id}`);

export const bulkDeletePackages = (ids: string[]) => api.post<{ deleted: number }>("/packages/bulk-delete", { ids });

export interface ProjectPackage extends Package {
  subject_name: string;
}

export const getPackageSummary = (id: string) => api.get<PackageSummary>(`/packages/${id}/summary`);
