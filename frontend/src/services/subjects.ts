...
export const createSubject = (data: CreateSubjectInput) => api.post<Subject>("/subjects", data);

export const updateSubject = (id: string, data: UpdateSubjectInput) => api.put<Subject>(`/subjects/${id}`, data);

export const deleteSubject = (id: string) => api.delete(`/subjects/${id}`);

export const bulkDeleteSubjects = (ids: string[]) => api.post<{ deleted: number }>("/subjects/bulk-delete", { ids });
