...
async def seed_project(db_conn: asyncpg.Connection) -> dict:
    ...
    row = await db_conn.fetchrow(
        "INSERT INTO projects (name, description, project_type, tags) "
        "VALUES ($1, $2, $3, $4) RETURNING *",
        "Test Project", "A test project", "atman", [],
    )
    return dict(row)
...
async def seed_subject(db_conn: asyncpg.Connection, seed_project: dict) -> dict:
    ...
    row = await db_conn.fetchrow(
        "INSERT INTO subjects (project_id, name, description, tags) "
        "VALUES ($1, $2, $3, $4) RETURNING *",
        seed_project["id"], "Test Subject", "A test subject", [],
    )
    return dict(row)
...
async def seed_asset(db_conn: asyncpg.Connection, seed_package: dict, seed_subject: dict) -> dict:
    ...
    row = await db_conn.fetchrow(
        ...
        seed_package["id"], seed_subject["id"],
        "frame_0001.png", "image", "aligned",
        1024, "/tmp/test/frame_0001.png", [], {},
    )
    return dict(row)
